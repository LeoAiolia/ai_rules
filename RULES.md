...
- 禁止魔法数字 / 硬编码字符串 / 硬编码颜色 / 硬编码尺寸。
- 常量必须通过 **枚举 / `const` / 配置对象 / 设计 Token** 定义。
...
- Dart 命名：**类用大驼峰**（`UserModel`），**变量/方法用小驼峰**（`fetchUser`），**常量用全小驼峰**（`maxRetryCount`）。
- Swift 命名：遵循 Swift API Design Guidelines，类型大驼峰，方法/属性小驼峰。
- 命名必须语义自解释，优先于写注释。

### 3.5 注释规范

- 注释只写**关键逻辑的"为什么"**：隐含约束、绕过特定 Bug 的原因、非直觉行为。
- 禁止注释描述代码"做了什么"（代码本身已表达）。
- 禁止无意义注释（`// 获取用户`、`// 返回结果`）。
