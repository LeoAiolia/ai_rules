...
├── RULES_COMPACT.md      # Trae 专用精简版（≤1000 字符，与 RULES.md 同步维护）
├── .ruler/
│   ├── AGENTS.md         # 由 sync.js 从 RULES.md 自动生成，ruler 读取此文件
...
```
RULES.md  →（sync.js）→  .ruler/AGENTS.md  →（ruler apply）→  目标项目规则文件
```

1. sync.js 将 `RULES.md` 内容（去除变更日志）写入 `.ruler/AGENTS.md`
...
```
RULES_COMPACT.md  →（sync.js）→  ~/.trae/user_rules/rules.md
```
...
```
修改 RULES.md（及 RULES_COMPACT.md）→  npm run sync  →  选择目标  →  输入项目目录  →  完成
```
...
1. 编辑 [`RULES.md`](./RULES.md)（完整规则）
2. 如有影响核心内容，同步编辑 [`RULES_COMPACT.md`](./RULES_COMPACT.md)（Trae 精简版）
3. 更新 `RULES.md` 顶部版本号、日期、文末变更日志
...
**Q：`.ruler/AGENTS.md` 和 `RULES.md` 有什么区别？**
A：内容基本相同，区别是 `.ruler/AGENTS.md` 由脚本自动生成，去除了变更日志（对 AI 无意义）。编辑时只修改 `RULES.md`。
